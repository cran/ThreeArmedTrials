...
The non-parameteric model does not assume any particular distribution for the data and $\mu_k$ and $\sigma^2_k$ are the expected value and the variance, respectively.
...
For the binarey, Poisson, and negative binomial model, the user can choose between an unrestricted maximum-likelihood variance estimator and a maximum-likelihood variance estimator restricted to the parameter space of the null hypothesis. 
The estimators $\hat{\lambda}_{k}$ are maximum-likelihood estimators for the parameteric models and group specific means for the non-parametric model.
...
The model is then specfied when calling the functions.
...
<a href="http://onlinelibrary.wiley.com/doi/10.1002/sim.2543/full">Kieser and Friede (2007)</a>.
...
Through the choice of the function $h$, the parameter $p_k$ can be transformed such that larger values of the parameter are more desireable. 
...
Then, the output lists of the numeric input parameters as well as the caculated total sample size `n` and the group sample sizes `nExp`, `nRef`, and `nPla`. <br><br>
...
The first element referes to the experimental treatment, the second to the reference, and the third to the placebo.
...
The dataset includes 32 patiens.
...
For the `seizures` data set non-inferiority of the experimental treatment compared to the reference will be tested with a non-inferiority margion of $\Delta=0.8$. 
...
* T. Mütze, A. Munk, T. Friede. *Design and analysis of three‐arm trials with negative binomially distributed endpoints*. Statistics in Medicine. 35:505-521 (2016a). 
* T. Mütze, F. Konietschke, T. Friede. *A studentized permutation test for three-arm trials in the `gold standard' design*. Submitted. (2016b). 
